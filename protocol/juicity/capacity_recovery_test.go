...
func newCapacityClient() *clientImpl {
	ctx, cancel := context.WithCancel(context.Background())
	return &clientImpl{ClientOption: &ClientOption{Ctx: ctx, Cancel: cancel}}
}
func TestCapacityDoesNotEstablishConnectionOnDataPlane(t *testing.T) {
	created := 0
	r := newClientRing(func(func(int64)) *clientImpl { created++; return newCapacityClient() }, 0)
	defer r.Close()
...
func TestReplenishmentKeepsHealthyMemberAvailable(t *testing.T) {
	r := newClientRing(func(func(int64)) *clientImpl { return newCapacityClient() }, 0)
	defer r.Close()
	member := &clientRingNode{cli: newCapacityClient(), capability: -1}
